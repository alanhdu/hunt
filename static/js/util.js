...
(function() {
  var displayArena, escape, formatNumber, getScoreboard, getType, height, pad, width;
  ...
  getType = function(char, arena, x, y) {
    ...
    if (char === '*') {
      type = 0;
      good = function(a, b) {
        return getPos(a, b) === '*' || getPos(a, b) === 'e';
      };
      if (good(x - 1, y) || good(x + 1, y)) {
        type += 1;
      }
      if (good(x, y - 1) || good(x, y + 1)) {
        type += 2;
      }
      return ((function() {
        switch (type) {
      ...
      visible = function(x, y) {
        return getPos(x, y) === '#' || getPos(x, y) === 'e';
      };
      if (visible(x - 1, y)) {
        if (visible(x + 1, y)) {
